...
from keyboards import (
    main_menu_kb, admin_menu_kb, categories_kb, products_kb,
    product_detail_kb, admin_products_kb, admin_product_detail_kb
)
...
async def edit_price_finish(message: Message, state: FSMContext, db: Database):
    ...
    product = await db.get_product_by_id(product_id)
    await message.answer(
        f"✅ Цена товара «{product.get('title')}» обновлена до {price}₽!",
        reply_markup=admin_menu_kb()
    )
...
@router.callback_query(F.data == 'back_admin_products')
