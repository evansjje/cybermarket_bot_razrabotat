...
class Database:
    ...
    async def add_user(self, user_id: int, username: str = "", first_name: str = "", last_name: str = "", referrer_id: int = 0) -> None:
        conn = await self.get_connection()
        try:
            await conn.execute(
    ...
    async def update_cart_count(self, user_id: int, product_id: int, count: int) -> None:
        conn = await self.get_connection()
        try:
            if count <= 0:
                await conn.execute(
    ...
    async def add_product(self, category_id: int, title: str, desc: str, price: float, file_data: str = "") -> int:
        conn = await self.get_connection()
        try:
            cursor = await conn.execute(
                """INSERT INTO products (category_id, title, desc, price, file_data)
...
# Сохраняем все существующие функции как обёртки для обратной совместимости
...
async def get_products_by_category(category_id: int) -> List[Dict[str, Any]]:
    db = Database()
    return await db.get_products_by_category(category_id)
...
async def add_to_cart(user_id: int, product_id: int, count: int = 1) -> None:
    db = Database()
    await db.add_to_cart(user_id, product_id, count)
...
async def update_cart_count(user_id: int, product_id: int, count: int) -> None:
    db = Database()
    await db.update_cart_count(user_id, product_id, count)
...
async def create_order(user_id: int, product_id: int, count: int, total_price: float) -> int:
    db = Database()
    return await db.create_order(user_id, product_id, count, total_price)
...
async def add_product(category_id: int, title: str, desc: str, price: float, file_data: str = "") -> int:
    db = Database()
    return await db.add_product(category_id, title, desc, price, file_data)
